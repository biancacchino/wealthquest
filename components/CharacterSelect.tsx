        ...
        {(Object.keys(CHARACTERS) as CharacterId[]).map((id) => (
          ...
          >
            <div className="w-32 h-32 bg-gray-800 border-4 border-white rounded-lg flex items-center justify-center mb-4 overflow-hidden">
               ...
               <img src={CHARACTERS[id].spriteSheet} alt={CHARACTERS[id].name} className="w-full h-full object-cover" />
            </div>
            <span style={{ color: CHARACTERS[id].color }} className="font-bold">{CHARACTERS[id].name}</span>
          </div>
        ))}
      </div>
    </div>
